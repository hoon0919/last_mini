    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="saveManuscriptDraftDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Author')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>글 임시 저장
                </v-btn>
                <v-dialog v-model="saveManuscriptDraftDialog" width="500">
                    <SaveManuscriptDraft
                        @closeDialog="saveManuscriptDraftDialog = false"
                        @saveManuscriptDraft="saveManuscriptDraft"
                    ></SaveManuscriptDraft>
                </v-dialog>
                <v-btn style="margin-left: 5px;" @click="saveManuscriptFinalDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Author')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>글 최종 저장
                </v-btn>
                <v-dialog v-model="saveManuscriptFinalDialog" width="500">
                    <SaveManuscriptFinal
                        @closeDialog="saveManuscriptFinalDialog = false"
                        @saveManuscriptFinal="saveManuscriptFinal"
                    ></SaveManuscriptFinal>
                </v-dialog>
            </div>
            <PublishingStatus @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></PublishingStatus>
...
export default {
    ...
    data: () => ({
        path: 'manuscripts',
        saveManuscriptDraftDialog: false,
        saveManuscriptFinalDialog: false,
    }),
    watch: {
    },
    methods:{
        async saveManuscriptDraft(params){
            try{
                var path = "saveManuscriptDraft".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','SaveManuscriptDraft 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.saveManuscriptDraftDialog = false
            }catch(e){
                console.log(e)
            }
        },
        async saveManuscriptFinal(params){
            try{
                var path = "saveManuscriptFinal".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','SaveManuscriptFinal 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.saveManuscriptFinalDialog = false
            }catch(e){
                console.log(e)
            }
        },
