        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:140px;">
                    <Number class="attributes-list" label="AuthorId" v-model="value.parameters.authorId" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        name: 'AuthorRegistrationDetails',
        components:{
        },
        props: {},
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'authors/search/authorRegistrationDetails',
                parameters: {}
            },
        }),
        created() {
