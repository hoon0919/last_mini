        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:140px;">
                    <Number class="attributes-list" label="PublicationHistoryId" v-model="value.parameters.publicationHistoryId" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        name: 'PublicationHistoryDetails',
        components:{
        },
        props: {},
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'publicationHistories/search/publicationHistoryDetails',
                parameters: {}
            },
        }),
        created() {
